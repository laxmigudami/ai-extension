...
class ChatUI {
    ...
    addMessage(content, type) {
        ...
        if (type === 'user') {
            ...
            const genType = eng === 'playwright' ? 'Playwright' : (eng === 'selenium' ? 'Selenium' : 'Test Automation');
            loader.innerHTML = `
              <div class="loading-spinner"></div>
    ...
    getPromptKeys(language, engine) {
        ...
        if (isFeatureChecked && isPageChecked) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                promptKeys.push('BEHAVE_WITH_PLAYWRIGHT_PYTHON_STEPS');
            } else {
    ...
    isPythonPlaywright(language, engine) {
        return (language === 'python' || language === 'py') && engine === 'playwright';
    }
    ...
    addUnsupportedLanguageMessage(language, engine) {
        const message = `⚠️ ${language}/${engine} combination is not yet supported. Currently supported: Java/Selenium, Python/Playwright.`;
